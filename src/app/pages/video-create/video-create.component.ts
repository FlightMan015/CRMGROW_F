...
export class VideoCreateComponent
  ...
  constructor(
    ...
    this.userService.garbage$.subscribe((_garbage) => {
      if (_garbage && _garbage._id) {
        const selectedTheme = this.themes.filter(
  ...
  finishUpload(type: string): void {
    if (
      ...
      if (this.selectedTheme) {
        this.video['theme'] = this.selectedTheme.id;
      }
      this.uploading = true;
      this.materialService
        ...
        .subscribe((res) => {
          if (res) {
            ...
            if (this.currentFolder) {
              this.router.navigate([
                `/materials/materials-list/${this.currentFolder}`
              ]);
            } else {
            ...
            if (res['data'] && res['data']['_id']) {
              this.themeSaveImpl(res['data']['_id'], this.selectedTheme.id);
            }
          }
        });
      return;
  ...
  updateVideo(video): void {
    ...
    if (this.selectedTheme) {
      newVideo['theme'] = this.selectedTheme.id;
    }
    ...
    this.materialService.uploadVideoDetail(videoId, newVideo).subscribe(
      (res) => {
        ...
        this.themeSaveImpl(videoId, this.selectedTheme.id);
      },
      (err) => {
        this.uploading = false;
        // this.toast.success(
  ...
  updatePdf(pdf): void {
    ...
    if (this.selectedTheme) {
      newPdf['theme'] = this.selectedTheme.id;
    }
    this.materialService.updatePdf(pdfId, newPdf).subscribe(
      (res) => {
        ...
        this.themeSaveImpl(pdfId, this.selectedTheme.id);
      },
      (err) => {
        this.uploading = false;
        // this.toast.success(
  ...
  updateImage(image): void {
    ...
    if (this.selectedTheme) {
      newImage['theme'] = this.selectedTheme.id;
    }
    this.materialService.updateImage(imageId, newImage).subscribe(
      (res) => {
        ...
        this.themeSaveImpl(imageId, this.selectedTheme.id);
      },
      (err) => {
        this.uploading = false;
        // this.toast.success(
    ...
    this.materialService
      ...
      .subscribe((res) => {
        ...
        if (res && res['data'] && res['data']['id']) {
          ...
          // Garbage Update with theme data
          this.themeSaveImpl(res['data']['id'], this.selectedTheme.id);
          if (this.currentFolder) {
            this.router.navigate([`/materials/${this.currentFolder}`]);
          } else {
            this.router.navigate(['/materials']);
