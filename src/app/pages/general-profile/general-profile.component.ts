  ...
  constructor(
    ...
    this.profileSubscription = this.userService.profile$.subscribe(
      (profile) => {
        ...
        const user_timezone = JSON.parse(profile.time_zone_info);

        this.timezones.some((ele) => {
          return ele.timezones.some((timezone) => {
            if (timezone.name === user_timezone?.name) {
              this.defaultTimeZone = ele.country !== 'OTHER';
              this.timezone = JSON.stringify(timezone);
              return true;
            }
            if (timezone.tz_name === user_timezone?.tz_name) {
              this.defaultTimeZone = ele.country !== 'OTHER';
              this.timezone = JSON.stringify(timezone);
              return true;
            }

            if (timezone.utc && timezone.utc.includes(user_timezone?.tz_name)) {
              this.defaultTimeZone = ele.country !== 'OTHER';
              this.timezone = JSON.stringify(timezone);
              return true;
            }
