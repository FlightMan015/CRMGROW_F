...
export class CampaignBulkMailingItemComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.routeSubscription = this.route.params.subscribe((params) => {
      this.id = params['id'];
      const prevPage = this.handlerService.previousUrl;
  ...
  loadMain(reload: boolean = false): void {
    if (reload) {
      this.reloading = true;
    } else {
      this.loading = true;
  ...
  goToBack(): void {
    ...
    } else if (this.handlerService.previousUrl.indexOf('/contacts') != -1) {
      this.handlerService.goBack(this.handlerService.previousUrl);
    } else {
      this.router.navigate(['campaign/bulk']);
    }
  }
  ...
  getPrevPage(): string {
    ...
    } else if (this.handlerService.previousUrl.indexOf('/contacts')) {
      return 'to Contact';
    } else {
      return 'to Bulk Mailing';
    }
  }
  ...
  cancelSession(session): void {
    ...
    if (session.status === 'awaiting') {
      message = 'Are you sure to remove this campaign session?';
      mode = 'remove';
          ...
          this.campaignService
            ...
            .subscribe((res) => {
              if (res && res['status']) {
                ...
                this.activities.contact.data = this.activities.contact.data.filter(
                  (e) => e._id !== contact._id
                );
                this.activities.awaiting.data = this.activities.awaiting.data.filter(
                  (e) => e._id !== contact._id
                );
                if (res['data']) {
