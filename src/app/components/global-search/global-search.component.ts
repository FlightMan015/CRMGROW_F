...
export class GlobalSearchComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.handlerService.searchStr$.subscribe((str) => {
      if (str) {
        ...
        this.materialLibraryLoadSubscription = this.storeService.libraries$.subscribe(
          (libraries) => {
        ...
        this.templateLibraryLoadSubscription = this.templateService.libraries$.subscribe(
          (libraries) => {
        ...
        this.automationLibraryLoadSubscription = this.automationService.libraries$.subscribe(
          (libraries) => {
        ...
        this.campaignLoadSubscription = this.campaignService.bulkLists$.subscribe(
          (campaigns) => {
  ...
  selectStage(stage: DealStage): void {
    ...
    this.dealsService.updatePipeLine(selPipeline[0]._id, { is_active: true }).subscribe();
    this.dealsService.selectedPipeline.next(selPipeline[0]);
    this.route.navigate(['/deals/stage/' + stage._id]);
  }
